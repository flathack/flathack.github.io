"""Render small ship icons (48x48 transparent PNGs) using the FLAtlas V2 decoder.

Usage:
    python render_ship_icons_v2.py <game_path> <mod_key> [limit]
"""
...
def render_icon(triangles: np.ndarray, out_path: Path) -> bool:
    ...
    square.paste(cropped, ((size - width) // 2, (size - height) // 2))
    square.resize((ICON_SIZE, ICON_SIZE), Image.LANCZOS).save(out_path, optimize=True)
    return True
...
def main() -> None:
    ...
    out_dir = Path(__file__).resolve().parent.parent / "data" / "ships-v2" / mod_key / "icons"
    ...
    print(f"Rendering {len(ship_nicks)} ship icons for [{mod_key}] with V2 -> {out_dir}")
    t0 = time.time()
    ok = 0
    skip = 0
    ...
    for idx, nick in enumerate(ship_nicks, 1):
        ...
        if render_icon(triangles, out_path):
            ok += 1
            print("OK")
        else:
            skip += 1
